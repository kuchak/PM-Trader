...
=======
                    # Use CLOB as primary price; use min for stop-loss safety
>>>>>>> Stashed changes
...
=======
                # Market resolved as a loss — record full loss and clean up
>>>>>>> Stashed changes
...
=======
                ...
                if token_id in self.open_positions:
                    mid = self.open_positions[token_id].get('market_id', '')
                    if mid in self.market_positions:
                        self.market_positions[mid] = max(0, self.market_positions[mid] - cost)
                        if self.market_positions[mid] <= 0:
                            del self.market_positions[mid]
                    self.closed_positions.append({
                        **self.open_positions[token_id], 'pnl': -cost, 'won': False,
                        'exit_price': 0, 'exit_ts': datetime.now(timezone.utc).isoformat()
                    })
                    del self.open_positions[token_id]
                self._api_exited.add(token_id)
                ...
                if token_id in self.open_positions:
                    ...
                    if mid in self.market_positions:
                        ...
                        if self.market_positions[mid] <= 0:
                            del self.market_positions[mid]
                    self.closed_positions.append({
                        **self.open_positions[token_id], 'pnl': pnl, 'won': action == "SELL_WIN",
                        'exit_price': sell_price, 'exit_ts': datetime.now(timezone.utc).isoformat()
                    })
                    del self.open_positions[token_id]
                self._api_exited.add(token_id)
...
=======
                # Market resolved on-chain — record appropriately
                pos = self.open_positions.get(token_id)
                if pos:
                    cost = pos['cost']
                    self.total_pnl -= cost
                    ...
                    del self.open_positions[token_id]
                self._api_exited.add(token_id)
                logger.info(f"  🏁 RESOLVED (API) | {outcome[:25]} | PnL: ${-cost:+.2f} — auto-cleaned")
                self._save()
>>>>>>> Stashed changes
